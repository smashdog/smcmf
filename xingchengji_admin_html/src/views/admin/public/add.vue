        ...
        <i-layout>
            ...
            <i-layout :style="{padding: '0 24px 24px'}">
                <i-content :style="{padding: '24px', minHeight: '100vh', background: '#fff'}">
                    <i-tabs style="max-width: 1024px;">
                        <i-tab-pane :label="'添加/编辑'+formTitle">
                            <i-form ref="addForm" :model="form" :rules="formDataVali" :label-width="100">
                                <i-form-item v-for="(v, k) in formData" :key="k" :label="v.title.split('：')[0]" :prop="k">
                                    <i-input v-if="v.type == 'password' || v.type == 'text' || v.type == 'textarea'" v-model="form[k]" :type="v.type"></i-input>
                                    <i-input v-if="v.type == 'number'" v-model="form[k]" type="text" :step="v.step"></i-input>
                                    <i-date-picker @on-open-change="setDateName(k)" @on-change="dateChange" v-if="v.type == 'date'" type="date" v-model="form[k]" format="yyyy-MM-dd" :show-week-numbers="true"></i-date-picker>
                                    <i-checkbox-group v-if="v.type == 'checkbox'" v-model="form[k]">
                                        <i-checkbox v-for="(v1, k1) in v.list" :key="k1" :label="v1.id">{{v1.title}}</i-checkbox>
                                    </i-checkbox-group>
                                    <i-radio-group v-if="v.type == 'radio'" v-model="form[k]">
                                        <i-radio v-for="(v1, k1) in v.list" :key="k1" :label="Number(v1.id)">{{v1.title}}</i-radio>
                                    </i-radio-group>
                                    ...
                                    <img v-if="v.type == 'img' && form[k]" :src="form[k]" alt="" style="max-width:100px;max-height:100px;">
                                    <i-upload
                                        v-if="v.type == 'img'"
                                        name="img"
                                    ...
                                    <i-upload
                                        v-if="v.type == 'file'"
                                        action="/index.php/admin/upload/file"
                                        name="file"
                                        :headers="token"
                                        :show-upload-list="false"
                                        >
                                ...
                                <i-form-item>
                                    <i-button type="success" @click="handleSubmit('addForm')">提交</i-button>
...
export default {
    ...
    created(){
        ...
        this.$axios.get('/index.php/admin/'+fullPath).then((res) => {
            if(res.data.status == '10000'){
                ...
                for(let k in this.formData){
                    if(/^\w+_ids$/.test(k)){
                        ...
                        this.$set(this.form, k, 0);
                    }else{
                        this.$set(this.form, k, '');
                    }
                    if(this.formData[k].type != 'checkbox' && this.formData[k].type != 'password'){
                        let temp1 = {
                            required: true,
                            message: (this.formData[k].type == 'radio' || this.formData[k].type == 'checkbox' || this.formData[k].type == 'select' ? '请选择' : '请填写')+this.formData[k].title,
                        ...
                        }else if(this.formData[k].type == 'radio' || this.formData[k].type == 'checkbox' || this.formData[k].type == 'select'){
                            temp1.type = 'number';
                        }
                ...
                if(typeof this.$route.query.id != 'undefined'){
                    ...
                    this.$axios.post('/index.php/admin/'+temp[0]+'/'+temp[0]+'Edit', params).then((res1) => {
    ...
    methods:{
        ...
        handleSubmit(name){
            ...
            this.$refs[name].validate((valid) => {
                if (valid) {
                    this.$submit('/index.php/admin/'+fullPath, this.form, (res) => {
                        this.$router.push({path: this.backUrl});
                    });
                } else {
